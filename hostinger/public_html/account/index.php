...
.oauth-wrap{display:flex;gap:10px;flex-wrap:wrap;margin:0 0 6px}
...
<div class="grid"><section class="stat"><span><?= ($bill['team_role'] ?? 'owner') === 'member' ? 'Team actions used' : 'Actions used' ?></span><?php if (!empty($usage['unlimited'])): ?><b>Unlimited actions</b><div class="meter"><span style="width:100%"></span></div><p>Admin access: usage is logged but never limited.</p><?php else: ?><b><?= $usage['used'] ?> / <?= $usage['limit'] ?></b><div class="meter"><span style="width:<?= $percent ?>%"></span></div><p><?= $usage['remaining'] ?> actions left in this period.<?= ($bill['team_role'] ?? 'owner') === 'member' ? ' Shared with ' . htmlspecialchars((string) $bill['team_owner_email']) . '\'s team.' : '' ?></p><?php endif; ?></section><section class="panel"><h2><?= $isMember ? 'Your billing' : 'Billing' ?></h2><p><?= $isMember ? 'This manages your own subscription only. The team plan is billed to the team owner.' : 'Update your payment method, download invoices, or change your subscription securely through Stripe.' ?></p>
<?php if (!$isMember && ($bill['plan'] ?? 'free') !== 'free' && !empty($bill['period_end'])): ?>
