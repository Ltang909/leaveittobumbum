...
if(t.role==='member'){box.innerHTML=`<p class="lede" style="margin:0"><img src="/bum/cat-hoodie.png" alt="Bum Bum in a hoodie" class="sticker" style="width:88px;vertical-align:middle;margin-right:10px">You're on <b>${esc(t.owner_email)}</b>'s team. Every action you complete draws from the shared bucket above.</p><p style="margin-top:14px"><button id="leaveTeam" class="button secondary">Leave team</button></p>`;const lv=box.querySelector('#leaveTeam');if(lv)lv.onclick=async()=>{if(!confirm('Leave this team? Your own plan (if any) takes over again.'))return;const s2=await session();await fetch('/api/team.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'leave',csrf:s2.csrf})});location.reload()};return}
...
box.querySelectorAll('[data-remove]').forEach(b=>b.onclick=async()=>{if(!confirm('Remove this teammate? They keep their own account, but lose access to your bucket.'))return;const s2=await session();await fetch('/api/team.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'remove',id:parseInt(b.dataset.remove,10),csrf:s2.csrf})});loadTeam()});
