...
</style></head><body><?php $showMeter = true; require dirname(__DIR__, 2) . '/includes/site-header.php'; ?><main class="shell"><p class="eyebrow">Bum Bum's toolbox</p><img class="tool-mascot-page" src="/bum/cat-wink.png" alt="Bum Bum winking"><h1>Never let a lead go cold.</h1><p class="lede">Rolodex is a tiny CRM for people who hate CRMs. Add the humans, move them down the pipeline, and every morning Rolodex tells you exactly who to follow up with and what to say. Adding a contact uses one action. Everything else is free.</p>
<?php if (!$user): ?><section class="panel"><h2>Sign in to use Rolodex</h2><a class="button" href="/account/?next=<?= urlencode('/tools/rolodex/') ?>">Sign in or create an account</a></section><?php else: ?>
...
async function apiCall(payload){const session=await fetch('/api/session.php').then(r=>r.json());const response=await fetch('/api/tools/rolodex.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...payload,csrf:session.csrf})});let data={};try{data=await response.json()}catch(e){}return{response,data}}
...
function contactCard(c){
  const deal=c.deal_value?` · ${money(c.deal_value)}`:'';
...
function bindCardButtons(root){
  ...
  root.querySelectorAll('[data-copy-draft]').forEach(b=>b.addEventListener('click',async()=>{
    ...
    bbTrack('rolodex_draft_copied',{tool:TOOL_KEY});
  }));
}
